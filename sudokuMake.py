...
def run(n = 40): ## GIVENS
    ...
    return flatten(best(all_results)), flatten(a_puzzle_solution)
...
def list_to_csv(lst, filename):
    with open(filename, 'wb') as f:
        ...
        strlist = [str(sublst)[1:-1] for sublst in lst]
        wr.writerow(strlist)

def generate(n, digits):
    all_puz = [run(digits) for i in range(n)]
    puzs, sols = zip(*all_puz)

    return list(puzs), list(sols)
...
#puzs, sols = generate(n=1)

#print([magic(puzs[0]), magic(sols[0])])

def worker(x):
    my_list = []
    counter = 0
    for i in range(0,1): #number of loops
        digits = random.randint(40,80)
        puzzles, solutions = generate(n=1, digits=digits) ## number of puzzles to generate
        blarg = [magic(puzzles[0]), magic(solutions[0])]
        my_list.extend(blarg)
        #list_to_csv(my_list, "mixedSudokus.csv")
        counter += 1
        if(counter%500 == 0):
            print(counter)
    if (x%1000 ==0):
        print("done with this process!", x)
    return my_list
...
longlist.extend(pool.map(worker, range(1000000)))
#pool.join()
#pool.close()

#print(longlist)

list_to_csv(longlist, "mixedSudokus.csv")

#print(magic(puzs[0]))
#list_to_csv(puzs, 'puzzles.csv')
#list_to_csv(sols, 'solutions.csv')
